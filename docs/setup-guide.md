...
The repository follows standard Home Assistant Add-on Store / HACS directory structure (`apps/ha-mcp-helper`):
...
4. Add your GitHub repository URL (e.g., `https://github.com/your-org/ha-ai`).
5. Click **Add** and then close the dialog.
6. The **Home Assistant AI Helper** add-on will appear in the Add-on Store under your custom repository.
...
3. Copy the `apps/ha-mcp-helper/` folder from this repository into `/addons/ha_mcp_helper`:
   ```bash
   # Example via SCP
   scp -r apps/ha-mcp-helper/ root@homeassistant.local:/addons/ha_mcp_helper
   ```
